...
def Track_Stitcher_v2(dataset,datasetidx_holder,labels):
    ...
    for a in range(len(CHQ_vec)):
	    ...
	    Length_between_vtx = distance.euclidean(vtx_A,vtx_B)
            ...
            projB_plus = vtx_B + np.asarray([1,1,1])*Length_between_vtx * np.asarray(local_PCB_b)
            projB_minus = vtx_B - np.asarray([1,1,1])*Length_between_vtx * np.asarray(local_PCB_b)
