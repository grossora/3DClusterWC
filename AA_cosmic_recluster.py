...
#######################################################################################
    ...
    if make_jsons:
	#make the sel holder from the ROI
	sel_idx = []
	for r in ROI_list:
	    if r[1] not in sel_idx:
	        sel_idx.append(r[1])
	    if r[2] not in sel_idx :
	        sel_idx.append(r[2])
        selidx_holder = [ showeridx_holder[x] for x in sel_idx]
        dh.MakeJson_Objects(dataset,selidx_holder,labels,jdir,jcount,'ROI_showers', mc_dl)
    #####################################################################
    #####################################################################
...
#######################################################################################
    ...
    # If length of ROI is == 1 do the rebase
